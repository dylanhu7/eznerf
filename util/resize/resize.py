...
def resize(
    ...
    images_directory = os.path.join(
        ...
        os.path.dirname(os.path.relpath(frames[0]["file_path"]))
        + f"_{size[0]}x{size[1]}",
    ...
    for frame in frames:
        image_path = os.path.join(
            os.path.dirname(path), os.path.relpath(frame["file_path"])
        )
        new_path = os.path.join(images_directory, os.path.basename(frame["file_path"]))
...
def main():
    ...
    parser.add_argument(
        "--image-suffix", type=str, default=None, help="suffix of images"
    )
    parser.add_argument(
        "--depth-suffix", type=str, default="_depth_0001", help="suffix of depth maps"
    )
    parser.add_argument(
        "--normal-suffix",
        type=str,
        default="_normal_0001",
        help="suffix of normal maps",
    )
    parser.add_argument(
        "--extension", type=str, default=".png", help="file extension of images"
    )
